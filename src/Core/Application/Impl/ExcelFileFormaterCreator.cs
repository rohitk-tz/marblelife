...
namespace Core.Application.Impl
{
    ...
    public class ExcelFileFormaterCreator : IExcelFileFormaterCreator
    {
        public IDictionary<int, CellValues> getColumnType()
        {
            ...
            dict.Add(3, CellValues.Number);
            dict.Add(4, CellValues.SharedString);
            dict.Add(5, CellValues.Number);
            dict.Add(6, CellValues.Number);
            dict.Add(7, CellValues.Number);
            dict.Add(8, CellValues.Number);
            dict.Add(9, CellValues.SharedString);
            dict.Add(10, CellValues.Number);
            dict.Add(11, CellValues.Number);
            dict.Add(12, CellValues.Number);
            dict.Add(13, CellValues.Number);
            return dict;
        }
        ...
        public List<String> ColumnNames()
        {
            return new List<string>
            {
                ...
                "Invoice Id",
                "Invoice Date",
                "Annual Sales Amount (in $)",
                "Annual Paid Amount (in $)",
                "Weekly Sales Amount (in $)",
                "Weekly Paid Amount (in $)",
        ...
        public bool CreateExcelDocument(List<AnnualGroupedReport> list, string xlsxFilePath)
        {
            int lastIndex = 0;
            var dt = new DataTable();
            dt = (ListToDataTable(list));
            DataTable ds = AddCoumns(list, dt);

            ds = AddRows(list, ds);
            return CreateExcelDocument(ds, xlsxFilePath);
        }
        public DataTable AddCoumns(List<AnnualGroupedReport> listElements, DataTable dt)
        {
            dt = SetColumns(listElements);
            return dt;
        }
        public DataTable AddRows(List<AnnualGroupedReport> listElements, DataTable dt)
        {
            var columnList = ColumnNames();
            foreach (var listGroupedElement in listElements)
            {
                DataRow _row = dt.NewRow();
                _row[0] = listGroupedElement.ReportTypeDescription;


                var isAnnualCell = listGroupedElement.ReportTypeDescription.Contains("ANNUAL SALES DATA:") ? true : false;
                dt.Rows.Add(_row);
                ...
                if (listGroupedElement.GroupedCollection.Count() == 0 && !isAnnualCell)
                {
                ...
                else
                {
                    if (!isAnnualCell)
                    {
                        foreach (var listElement in listGroupedElement.GroupedCollection)
                        {
                            var date = listElement.InvoiceDate.ToString("MM/dd/yyyy");
                            DataRow _row2 = dt.NewRow();
                            _row2[columnList[0]] = listElement.CustomerName;
                            _row2[columnList[1]] = listElement.QbInvoice;
                            _row2[columnList[2]] = listElement.InvoiceId;
                            _row2[columnList[3]] = date;
                            _row2[columnList[4]] = listElement.AnnualSalesAmount;
                            _row2[columnList[5]] = listElement.AnnualPaidAmount;
                            _row2[columnList[6]] = listElement.WeeklySalesAmount;
                            _row2[columnList[7]] = listElement.WeeklyPaidAmount;
                            _row2[columnList[8]] = listElement.ReportTypeDescription;
                            _row2[columnList[9]] = listElement.AnnuallyDifference;
                            _row2[columnList[10]] = listElement.WeeklyDifference;
                            _row2[columnList[11]] = listElement.AnnuallySalesDifference;
                            _row2[columnList[12]] = listElement.AnnuallyPaidDifference;
                            dt.Rows.Add(_row2);
                        }
                    }
                }
        ...
        private DataTable SetColumns<T>(List<T> list)
        {
        ...
        public bool CreateExcelDocument(DataTable dt, string excelFilename)
        {
            excelFilename = MediaLocationHelper.GetTempMediaLocation().Path + "/annualSalesData-" + string.Format("{0:yyyy-MM-dd_hh-mm-ss-tt}", DateTime.Now) + ".xlsx";
        ...
        private void CustomizedCode(WorksheetPart worksheetPart1, SharedStringTablePart sharedStringTablePart1, DataTable dt)
        {
            ...
            Pane pane1 = new Pane() { HorizontalSplit = 4D, VerticalSplit = 1D, TopLeftCell = "A2", ActivePane = PaneValues.BottomLeft, State = PaneStateValues.Frozen };
        ...
        private Columns GetColumns(int totalColumns)
        {
            ...
            for (int a = 0; a < totalColumns; a++)
            {
                ...
                Column columnName = new Column() { Min = (UInt32Value)1U, Max = (UInt32Value)1U, Width = 100.52D, Style = (UInt32Value)1U, Hidden = false, CustomWidth = true, OutlineLevel = 0, Collapsed = false };
        ...
        private SheetData AppendRows(DataTable dt)
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                ...
                else if (cell == "Under Reporting Candidate" || cell == "Over Reporting Candidate" || cell == "NON ROYALTY REPORTING CLASS" || cell == "Missing Report"
                        || cell == "Missing Payment Report" || cell == "REVIEW INDIVIDUALLY" || cell == "SPLIT INVOICES" || cell == "Other Types" || cell == "REVIEW - NO ACTION" || cell == "ROUNDING")
                {
                    row1 = new Row() { RowIndex = (UInt32Value)rowIndex, CustomFormat = true, Height = 32.8D, Hidden = false, CustomHeight = true, OutlineLevel = 0, Collapsed = false };
                    cellStyleIndex = 6U;
                }
