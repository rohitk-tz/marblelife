...
{
    public static class HomeAdvisorFileParser
    {
        ...
        private static bool CheckSheetHasRequiredColumns(Worksheet workSheet, SpreadsheetDocument document, out int startingIndex, out int startingRow)
        {
            ...
            foreach (Row row in rows)
            {
                ...
                if (coll.Count() > 0)
                {
                    if (coll.Contains("ha account") && coll.Contains("company name") && coll.Contains("sr id")
                        && coll.Contains("sr submitted date") && coll.Contains("task") && coll.Contains("net lead $")
                        && coll.Contains("city") && coll.Contains("state") && coll.Contains("zip code")
                        && coll.Contains("lead type"))
                    {
