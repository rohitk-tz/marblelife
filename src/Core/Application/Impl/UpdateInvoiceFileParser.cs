...
{
    public static class UpdateInvoiceFileParser1
    {
        ...
        public static List<string> GetSheetIds(string path)
        {
            ...
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Open(path, false))
            {
                ...
                var sheetName = sheets.FirstOrDefault(x => x.Name.ToString().ToUpper().StartsWith("COUNTY"));
                if (sheetName != default(Sheet))
                {
                    sheetIds.Add(sheets.FirstOrDefault(x => x.Name.ToString().ToUpper().StartsWith("COUNTY")).Id);
                }

                sheetName = sheets.FirstOrDefault(x => x.Name.ToString().ToUpper().StartsWith("ZIP")); ;
                if (sheetName != default(Sheet))
                {
                    sheetIds.Add(sheets.FirstOrDefault(x => x.Name.ToString().ToUpper().StartsWith("ZIP")).Id);
                }
