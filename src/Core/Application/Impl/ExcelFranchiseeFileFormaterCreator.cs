...
namespace Core.Application.Impl
{
    ...
    public class ExcelFranchiseeFileFormaterCreator : IExcelFranchiseeFileFormaterCreator
    {
        ...
        public bool CreateExcelDocument(List<FranchiseeModel> list, string xlsxFilePath)
        {
            int lastIndex = 0;
        ...
        public DataTable AddCoumns(List<FranchiseeModel> listElements, DataTable dt)
        {
            dt = SetColumns(listElements);
            return dt;
        }
        public DataTable AddRows(List<FranchiseeModel> listElements, DataTable dt)
        {
            var columnList = ColumnNames();
            foreach (var listGroupedElement in listElements)
            {
                DataRow _row = dt.NewRow();
                _row[0] = listGroupedElement.CategoryName;


                var isAnnualCell = listGroupedElement.CategoryName.Contains("ANNUAL SALES DATA:") ? true : false;
                dt.Rows.Add(_row);
