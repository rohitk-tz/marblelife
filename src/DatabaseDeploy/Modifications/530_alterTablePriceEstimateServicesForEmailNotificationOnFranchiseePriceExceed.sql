ALTER TABLE `priceestimateservices` 
ADD COLUMN `IsFranchiseePriceExceed` bit(1) default false;

ALTER TABLE `priceestimateservices` 
ADD COLUMN `IsFranchiseePriceExceedForEmail` bit(1) default false;


insert into notificationtype(Title, Description, IsQueuingEnabled, IsServiceEnabled, IsDeleted)
values('Notification To Super Admin If Franchisee Price Exceeds The Bulk Corporate Price', 'Notification To Super Admin If Franchisee Price Exceeds The Bulk Corporate Price', 1, 1, 0);

Insert into emailtemplate (Id, NotificationTypeId, Title, Description, Subject, Body, IsRequired, IsDeleted, languageId, isActive)
Values (578, 58, 'Notification To Super Admin If Franchisee Price Exceeds The Bulk Corporate Price', 'Notification To Super Admin If Franchisee Price Exceeds The Bulk Corporate Price', 
'Notification To Super Admin If Franchisee Price Exceeds The Bulk Corporate Price',
'<!DOCTYPE html
