(function () {
    angular.module(SchedulerConfiguration.moduleName).controller("UploadPriceEstimateController",
        ["$scope", "$rootScope", "$state", "$q", "$uibModalInstance", "FranchiseeService",
            "ManagePriceEstimateService", "FileService", "Toaster", "Clock", "Notification", "APP_CONFIG",
            function ($scope, $rootScope, $state, $q, $uibModalInstance, franchiseeService,
                priceEstimateService, fileService, toaster, clock, notification, config) {
                ...
                function saveFile() {
                    vm.fileInfo.isFranchiseeAdmin = vm.isFranchiseeAdmin;
                    return priceEstimateService.saveFile(vm.fileInfo).then(function (result) {
