...
angular.module(SchedulerConfiguration.moduleName).directive('signaturePad', ['$interval', '$timeout', '$window',
    function ($interval, $timeout, $window) {
        ...
        return {
            restrict: 'EA',
            replace: true,
            template: '<div class="signature" style="width: 100%; max-width:{{width}}px; height: 100%; max-height:{{height}}px;border:1px solid;border-color:blue !important"><canvas class="colors_sketch" style="display: block; margin: 0 auto;" ng-mouseup="onMouseup()" ng-mousedown="notifyDrawing({ drawing: true })"></canvas></div>',
            scope: {
                accept: '=?',
                clear: '=?',
                disabled: '=?',
...
angular.module('ngSignaturePad', [SchedulerConfiguration.moduleName]);
