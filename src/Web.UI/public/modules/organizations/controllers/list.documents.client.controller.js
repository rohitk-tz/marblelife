(function () {
    ...
    angular.module(OrganizationsConfiguration.moduleName).controller("ManageDocumentsController",
        ["$scope", "$rootScope", "FranchiseeService", "FileService", "Notification", "APP_CONFIG", "$uibModal",
        ...
        function ($scope, $rootScope, franchiseeService, fileService, notification, config, $uibModal,
                    ...
                    resolve: {
                        modalParam: function () {
                            return {
                                ...
                                CategoryId: vm.query.categoryId,
                                DocumentId: vm.docId,
                                UploadFrom: vm.UploadFrom
                            };
                        }
                    }
                });
            ...
            function getdocumentTypeForFranchisee() {
                return franchiseeDocumentService.getDocumentTypeForFranchisee(vm.query.franchiseeId).then(function (result) {
                    vm.documentType = result.data;
                    if (vm.info!=null && vm.info.id != 0) {
                        var document = $filter('filter')(vm.documentType, { value: vm.documentInfo.documentTypeId.toString() }, true);
                        if (document[0] != undefined) {
                            vm.documentTypeId = document[0].value.toString();
                        }
                    }
                });
            }

            $scope.$emit("update-title", "Franchisee Management Documents");

            $q.all([getDocumentList(), getFranchiseeCollection(), prepareSearchOptions(), getdocumentTypeForFranchisee()]);
        }]);
}());