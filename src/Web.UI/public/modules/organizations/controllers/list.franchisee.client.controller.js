(function () {
    ...
    angular.module(OrganizationsConfiguration.moduleName).controller("ListFranchiseeController",
        ["$state", "$scope", "$rootScope", "FranchiseeService", "$q", "APP_CONFIG",
            "$uibModal", "Notification", "Toaster", "LocalStorageService",
            "FileService", "URLAuthenticationServiceForEncryption",
            function ($state, $scope, $rootScope, franchiseeService, $q, config, $uibModal, notification, toaster, LocalStorageService, fileService,
                ...
                vm.getDeactivationCode= getDeactivationCode;
                ...
                function getFranchiseeList() {
                    return franchiseeService.getFranchiseeCollection(vm.query).then(function (result) {
                        if (result != null && result.data != null) {
                            ...
                            vm.franchiseeListForPrint = angular.copy(vm.franchiseeList);
                ...
                function getDeactivationCode(franchiseeId) {
                    var modalInstance = $uibModal.open({
                        animation: true,
                        templateUrl: 'modules/organizations/views/show-deactivation-note.client.view.html',
                        controller: 'ShowDeactivationNoteController',
                        controllerAs: 'vm',
                        size: 'lg',
                        backdrop: 'static',
                        resolve: {
